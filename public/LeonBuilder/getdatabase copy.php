...
if (isset($_GET['all'])) {
    ...
    foreach ($tableList as $val) {
        ...
        $db_name = str_replace("tw_", "", $db_name);
        $Save_DB = ["db_note" => $db_name, "db_name" => $db_name, "db_data" => json_encode($tableListArr, JSON_UNESCAPED_UNICODE), 'other_data' => json_encode($other_data, JSON_UNESCAPED_UNICODE)];
        $Panda_Class->_INSERT("leon_database", $Save_DB);
    }
    $tableList = [];
}
...
<body>
    ...
    function Create() {
        location.href = window.location.pathname + '?all=true'
    }
