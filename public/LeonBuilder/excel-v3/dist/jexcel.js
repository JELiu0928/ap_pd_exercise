...
var jexcel = (function(el, options) {
        ...
        for (var j = 0; j < y; j++) {
            ...
            for (var i = 0; i < x; i++) {
                ...
                if (! highlighted || obj.records[j][i].classList.contains('highlight')) {
					...
					element[obj.options.columns[i].key] = obj.options.data[j][i];
                    //dataset[py][obj.options.columns[i].key] = obj.options.data[j][i];
                    px++;
                }
            }
            if (px > 0) {
    ...
    obj.getRowCount = function() {
        return obj.options.data.length - 1;;
    }
    ...
    obj.createRow = function(j, data) {
		...
		//td.setAttribute('data-key', obj.options.columns[i].key);
        // Definitions
    ...
    obj.createCellHeader = function(colNumber) {
        ...
        obj.headers[colNumber].setAttribute('data-key', obj.options.columns[colNumber].key);
