...
<head itemscope="itemscope" itemtype="http://schema.org/WebSite">
    ...
    {{-- 圖像 --}}
    @if (isset($basic_seo['seo_img']) && $basic_seo['seo_img'] != '/noimage.svg')
        <meta property="og:image" content="{{ URL::to('/') . $basic_seo['seo_img'] }}" />
        <meta itemprop="image" content="{{ URL::to('/') . $basic_seo['seo_img'] }}" />
        <meta name="twitter:card" content="{{ URL::to('/') . $basic_seo['seo_img'] }}" />
        <meta name="twitter:image:src" content="{{ URL::to('/') . $basic_seo['seo_img'] }}" />
    @endif
    {{-- LD Json --}}
...
<body class="@yield('bodyClass')" data-overlayscrollbars-initialize
    style="--gradient-start: #00AAE8; --gradient-end: #00E8A2">
