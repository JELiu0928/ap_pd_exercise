...
class CreateBackendMainTypesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        collect(['tw_', 'en_'])->map(function($locale){
            Schema::create( $locale . 'backend_main_types', function (Blueprint $table) {
    ...
    {
        collect(['tw_', 'en_'])->map(function($locale){
            Schema::dropIfExists( $locale . 'backend_main_types');
        });
    }
}
