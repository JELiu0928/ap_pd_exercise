...
class CreateMainWithCategoriesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        collect(['tw_', 'en_'])->map(function($locale){
            Schema::create( $locale . 'main_with_category', function (Blueprint $table) {
    ...
    {
        collect(['tw_', 'en_'])->map(function($locale){
            Schema::dropIfExists( $locale . 'main_with_category');
        });
    }
}
