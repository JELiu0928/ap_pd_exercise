...
class ProductSeriesApi extends CmsApi implements GetTable, GetExport
{
    use BasicSql, BasicRole;
    protected $FilterField = '{FilterField}';

    protected $modelArray = [
        'ProductSeries' => ProductSeries::class,
    ...
    protected function formatBuilder(string $modelClass, Builder $builder): Builder
    {
        ...
        if ($modelClass === ProductSeries::class) {
    ...
    {
        $sql = $this->formatBuilder(ProductSeries::class, $this->basicSql(ProductSeries::class, $this->req->search));
        ...
        $colSetting = ColumnSet::make()
            ...
            ->selectCol('category_id', '所屬類別', M('ProductCategory')::getList()->toArray(), 200)
        ...
        return TableResponse::create($colSetting, 'ProductSeries', $data, $this->cmsMenu, $role, '系列管理', (int) $totalPage, (int) $site_page, (int) $perCount, (int) $totalCount);
    }
    public function getExport(): ExportResponse
    {
        $sql = $this->basicSql(ProductSeries::class);
        $data = (clone $sql)->get();
